...
const cmsService = {
    ...
    getAdsList: async (ctx, adspaceId) => {
        ...
        for (let i = 0; i < banners.jDtos.length; i++) {
            ...
            banners.jDtos[i].attrAdName = banners.jDtos[i].adName.split('-')[1];
        }
    ...
    //build面包屑
    buildCrumbs: function (status, obj, isAgentNews) {
        let crumbs = [];
        if (!isAgentNews) {
            ...
            crumbs.push(new NewsCrumb('/agent', '首页'));
            if (status == 'channl' && obj && obj.channelId) {
                // crumbs.push(new NewsCrumb('/agentNews', '资讯中心'));
                crumbs.push(new NewsCrumb('/agentNews/c' + obj.channelId + '-' + 1, obj.channelName, true));
            } else if (status == 'tag' && obj) {
                // crumbs.push(new NewsCrumb('/agentNews', '资讯中心'));
                crumbs.push(new NewsCrumb('/agentNews/tag' + obj.tagName + '-' + 1, obj.tagName, true));
            } else if (status == 'content' && obj && obj.channelId) {
                // crumbs.push(new NewsCrumb('/agentNews', '资讯中心'));
                crumbs.push(new NewsCrumb('/agentNews/c' + obj.channelId + '-' + 1, obj.channelName));
                crumbs.push(new NewsCrumb('/agentNews/d' + obj.contentId + '.html', obj.title, true));
            } else if (status == 'special' && obj) {
                // crumbs.push(new NewsCrumb('/agentNews', '资讯中心'));
                crumbs.push(new NewsCrumb('/agentNews/special' + obj.channelId + '-' + 1, obj.channelName, true));
            } else {
                // crumbs.push(new NewsCrumb('/agentNews', '资讯中心', true));
            }
        }
